    ...
    namespace ldso {
        ...
        bool LoopClosing::DetectLoop(shared_ptr<Frame> &frame) {
            ...
            if (r.Score < minScoreAccept) {
                ...
                checkedKFs[id] = frame;
                candidateKF = checkedKFs[r.Id];
                LOG(INFO) << "add loop candidate from " << candidateKF->kfId << ", current: " << frame->kfId
                          << ", score: "
                          << r.Score << endl;
                return true;
            }
        ...
        bool LoopClosing::ComputeOptimizedPose(shared_ptr<Frame> pKF, Sim3 &Scr, shared_ptr<CalibHessian> Hcalib,
            ...
            if (ScrOpti.scale() == Scr.scale() || std::isnan(ScrOpti.scale()) ||
                ScrOpti.scale() < 0)  // optimization failed
                return false;
