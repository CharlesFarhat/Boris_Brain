    ...
    namespace ldso {
        ...
        namespace internal {
            ...
            void EnergyFunctional::insertFrame(shared_ptr<FrameHessian> fh, shared_ptr<CalibHessian> Hcalib) {
                ...
                for (auto fh2: frames) {
                    connectivityMap[(((uint64_t) fh->frameID) << 32) + ((uint64_t) fh2->frameID)] = Eigen::Vector2i(0,
                                                                                                                    0);
                    if (fh2 != fh)
                        connectivityMap[(((uint64_t) fh2->frameID) << 32) + ((uint64_t) fh->frameID)] = Eigen::Vector2i(
                                0,
                                0);
                }
            }
            ...
            void EnergyFunctional::solveSystemF(int iteration, double lambda, shared_ptr<CalibHessian> HCalib) {
                ...
                if (setting_solverMode & SOLVER_SVD) {
                    ...
                    VecX SVecI = (HFinal_top.diagonal() +
                                  VecX::Constant(HFinal_top.cols(), 10)).cwiseSqrt().cwiseInverse();
                    MatXX HFinalScaled = SVecI.asDiagonal() * HFinal_top * SVecI.asDiagonal();
            ...
            void EnergyFunctional::setDeltaF(shared_ptr<CalibHessian> HCalib) {
                ...
                for (int h = 0; h < nFrames; h++)
                    for (int t = 0; t < nFrames; t++) {
                        int idx = h + t * nFrames;
                        adHTdeltaF[idx] =
                                frames[h]->get_state_minus_stateZero().head<8>().cast<float>().transpose() *
                                adHostF[idx]
                                +
                                frames[t]->get_state_minus_stateZero().head<8>().cast<float>().transpose() *
                                adTargetF[idx];
                    }
            ...
            void EnergyFunctional::setAdjointsF(shared_ptr<CalibHessian> Hcalib) {
                ...
                for (int h = 0; h < nFrames; h++)
                    for (int t = 0; t < nFrames; t++) {
                        ...
                        Vec2f affLL = AffLight::fromToVecExposure(host->ab_exposure, target->ab_exposure,
                                                                  host->aff_g2l_0(),
            ...
            void
            EnergyFunctional::resubstituteFPt(const VecCf &xc, Mat18f *xAd, int min, int max, Vec10 *stats, int tid) {
