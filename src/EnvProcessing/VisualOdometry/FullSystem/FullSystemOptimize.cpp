...
namespace Boris_Brain {
	namespace dso {
		...
		void
		FullSystem::linearizeAll_Reductor(bool fixLinearization, std::vector<PointFrameResidual *> *toRemove, int min,
...
//			good, bad);
		...
		Vec3 FullSystem::linearizeAll(bool fixLinearization) {
			...
			if (multiThreading) {
				treadReduce.reduce(
						boost::bind(&FullSystem::linearizeAll_Reductor, this, fixLinearization, toRemove, _1, _2, _3,
									_4), 0, activeResiduals.size(), 0);
...
// applies step to linearization point.
		bool
		FullSystem::doStepFromBackup(float stepfacC, float stepfacT, float stepfacR, float stepfacA, float stepfacD) {
...
// sets linearization point.
		...
		float FullSystem::optimize(int mnumOptIts) {
			...
			for (int iteration = 0; iteration < mnumOptIts; iteration++) {
				...
				double incDirChange =
						(1e-20 + previousX.dot(ef->lastX)) / (1e-20 + previousX.norm() * ef->lastX.norm());
				previousX = ef->lastX;
